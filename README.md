...
Поднимется на `http://localhost:5173` и обращается к API на `http://localhost:8000`. Убедись, что бэкенд уже запущен (см. выше) и что `CORS_ALLOWED_ORIGINS` в `.env` включает `http://localhost:5173` (по умолчанию так и есть). Панель не подключена к `docker-compose.yml` — запускается отдельно через Vite dev-сервер.
